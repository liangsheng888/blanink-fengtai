...
public class AfterSaleNotHandle extends Fragment {
    ...
    private final static int HANDLE_CODE = 100;
        ...
        lvAfterSale.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position < rowsBeanList.size()) {
                    ...
                    intent.putExtra("id", rowsBeanList.get(position).getId());
                    startActivityForResult(intent, HANDLE_CODE);
                }
            }
        });
    }
    ...
    public void loadData() {
        ...
        RequestBody body = new FormBody.Builder().add("userId", sp.getString("USER_ID", null))
                .add("my", "0").add("pageNo", pageNo + "").build();
