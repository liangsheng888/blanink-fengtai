...
public class RemarkGoOrderReview extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_remark_go_order_review);
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (rr.getResult().size() > 0) {
                            lvReview.setAdapter(new CommonAdapter<ReviewReply.ResultBean.ReviewReplyListBean>(RemarkGoOrderReview.this, rr.getResult().get(0).getReviewReplyList(), R.layout.item_review_detail) {
                                @Override
                                public void convert(ViewHolder viewHolder, ReviewReply.ResultBean.ReviewReplyListBean reviewReplyListBean, int position) {
                                    ...
                                    GlideUtils.glideImageView(RemarkGoOrderReview.this, iv, reviewReplyListBean.getCompany().getPhoto(),true);
                            ...
                            lvReviewSelf.setAdapter(new CommonAdapter<ReviewReply.ResultBean.ReviewReplyListBean>(RemarkGoOrderReview.this, rr.getResult().get(1).getReviewReplyList(), R.layout.item_review_detail) {
                                @Override
                                public void convert(ViewHolder viewHolder, ReviewReply.ResultBean.ReviewReplyListBean reviewReplyListBean, int position) {
                                    reviewReplyListBean = rr.getResult().get(0).getReviewReplyList().get(position);
                                    ImageView iv = viewHolder.getViewById(R.id.iv);
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (review.getResult().getComments() != null) {
                            tvService.setText(review.getResult().getComments().getServiceGrade());
                            tvPay.setText(review.getResult().getComments().getQualityGrade());
                            tvAtt.setText(review.getResult().getComments().getTimingGrade());
                            ...
                            GlideUtils.glideCornerImageView(RemarkGoOrderReview.this, ivOther, review.getResult().getComments().getCompany().getPhoto());
                        }
                        if (review.getResult().getReview() != null) {
                            tvEct.setText(review.getResult().getReview().getServiceGrade());//态度
                            tvPayTime.setText(review.getResult().getReview().getPaymentGrade());//付款及时
                            tvCost.setText(review.getResult().getReview().getIntegrityGrade());//慷慨
                            ...
                            GlideUtils.glideCornerImageView(RemarkGoOrderReview.this, iv, review.getResult().getReview().getCompany().getPhoto());
                        }
                    }
                });
            }
                    ...
                    public void run() {
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                Toast.makeText(RemarkGoOrderReview.this, "服务器异常", Toast.LENGTH_SHORT).show();
                            }
                        });
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if("00000".equals(re.getErrorCode())){
                            ...
                            dialogFlow.hideProgressdialog();
                            Toast.makeText(RemarkGoOrderReview.this, "回复失败", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
