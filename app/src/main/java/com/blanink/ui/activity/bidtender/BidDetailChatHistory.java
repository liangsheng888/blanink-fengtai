 ...
 * 投标详情 历史回复记录
 */
public class BidDetailChatHistory extends BaseActivity implements View.OnClickListener {
    ...
    public void loadMessagetoServer(String inputText) {
        ...
        if (oldTalkHistoryLog.result.rows.size() == 0) {
            ...
            mapParams.put("receiveUser", oldTalkHistoryLog.result.rows.get(0).createBy.id);
            Log.e("BidDetailChatHistory", "senddata:[" + "createBy：" + oldTalkHistoryLog.result.rows.get(0).createBy.id + "]");
        }
