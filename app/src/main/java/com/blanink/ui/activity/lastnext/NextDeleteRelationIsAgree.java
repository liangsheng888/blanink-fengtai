...
public class NextDeleteRelationIsAgree extends BaseActivity {
    @BindView(R.id.title)
    TextView title;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_next_delete_relation_is_agree);
        ...
        RetrofitFactory.getApiService(this)
                ...
                .subscribe(new Observer<NotifyOffice>() {
                    ...
                    public void onNext(NotifyOffice office) {
                        ...
                        GlideUtils.glideImageView(NextDeleteRelationIsAgree.this, iv, notifyOffice.getResult().getCompanyA().getPhoto(), true);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.bt_agree:
                //同意
                deleteNofity("你们将解除合作关系,请谨慎!","取消","确定");
                break;
            case R.id.bt_refuse:
                //拒绝
                showNotify();
            ...
            RetrofitFactory.getApiService(this)
                    ...
                    .subscribe(new Observer<Response>() {
                        ...
                        public void onError(Throwable e) {
                            ...
                            Toast.makeText(NextDeleteRelationIsAgree.this, "服务器异常", Toast.LENGTH_SHORT).show();
                        ...
                        public void onNext(Response response) {
                            ...
                            if ("00000".equals(response.getErrorCode())) {
                                ...
                                params.put("flag","REJECT_DEL_SUPPLIER");

                                RetrofitFactory.getApiService(NextDeleteRelationIsAgree.this)
                                ...
                                DialogNotifyUtils.showNotify(NextDeleteRelationIsAgree.this, "你拒绝了对方的请求");
                            } else {
                                Toast.makeText(NextDeleteRelationIsAgree.this, "操作失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
    }
                ...
                .subscribe(new Observer<Response>() {
                    ...
                    public void onNext(Response response) {
                        ...
                        if ("00000".equals(response.getErrorCode())) {
                            ...
                            params.put("currentUser.id",sp.getString("USER_ID", null));
                            params.put("office.id",notifyOffice.getResult().getCompanyA().getId());
                            params.put("flag","AGREE_DEL_SUPPLIER");
                            ...
                            DialogNotifyUtils.showNotify(NextDeleteRelationIsAgree.this, "你同意了对方的解除请求");
                        } else {
                            Toast.makeText(NextDeleteRelationIsAgree.this, "操作失败", Toast.LENGTH_SHORT).show();
                        }
                    }
    ...
    private void deleteNofity( String content, String left, String right) {
        ...
        window.findViewById(R.id.tv_exit).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DialogLoadUtils.getInstance(NextDeleteRelationIsAgree.this);
                DialogLoadUtils.showDialogLoad("操作进行中...");
                agreeApply();
                alertDialog.dismiss();
            }
