 ...
 * 评论去单订单
 */
public class RemarkGoOrder extends BaseActivity {
    ...
    MyRatingBar rbQuality;
    @BindView(R.id.tv_time_ef)
    TextView tvTimeEf;
    @BindView(R.id.rb_time_ef)
    MyRatingBar rbTimeEf;
    ...
    @BindView(R.id.tv_kk_self)
    TextView tvKkSelf;
    @BindView(R.id.rb_kk_self)
    MyRatingBar rbKkSelf;
    ...
    private float totalTime = 0.0f;//时效

    private float totalKKSelf = 0.0f;

    private float totalCostSelf = 0.0f;//付款及时

    private float totalQuality = 0.0f;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_remark_go_order);
    ...
    private void initData() {
        ...
        rbQuality.setOnRatingChangeListener(new MyRatingBar.OnRatingChangeListener() {
            @Override
            public void onRatingChange(float ratingCount) {
                totalQuality = ratingCount * oneGrade;
            }
        });
        rbTimeEf.setOnRatingChangeListener(new MyRatingBar.OnRatingChangeListener() {
            @Override
            public void onRatingChange(float ratingCount) {
                totalTime = ratingCount * oneGrade;
            }
        });
        //(自我检讨)
        ...
        rbKkSelf.setOnRatingChangeListener(new MyRatingBar.OnRatingChangeListener() {
            @Override
            public void onRatingChange(float ratingCount) {
                totalKKSelf = ratingCount * oneGrade;
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (totalService == 0.0f && totalQuality == 0.0f && totalTime == 0.0f && TextUtils.isEmpty(strOther) && totalServiceSelf == 0.0f && totalKKSelf == 0.0f && totalCostSelf == 0.0f && totalCostSelf == 0.0f && TextUtils.isEmpty(strSelf)) {
                    Toast.makeText(RemarkGoOrder.this, "你没有做任何评论操作", Toast.LENGTH_SHORT).show();
                    return;
                }
                DialogLoadUtils.getInstance(RemarkGoOrder.this);
    ...
    private void saveRemarkData(String strOther, String strSelf) {
        ...
        RequestBody body = new FormBody.Builder()
                ...
                .add("review.integrityGrade", totalKKSelf == 0.0f ? "" : totalKKSelf + "")
                .add("review.paymentGrade", totalCostSelf == 0.0f ? "" : totalCostSelf + "")
                .add("review.contents", strSelf)
                .add("comments.serviceGrade", totalService == 0.0f ? "" : totalService + "")
                .add("comments.qualityGrade", totalQuality == 0.0f ? "" : totalQuality + "")
                .add("comments.timingGrade", totalTime == 0.0f ? "" : totalTime + "")
        ...
        ok.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (review.getResult().getComments() != null) {
                            rbTimeEf.setClickable(false);

                            rbQuality.setClickable(false);
                            rbService.setClickable(false);
                            etOther.setClickable(false);
                            rbTimeEf.setStar(Float.parseFloat(review.getResult().getComments().getTimingGrade()));
                            rbQuality.setStar(Float.parseFloat(review.getResult().getComments().getQualityGrade()));
                        ...
                        if (review.getResult().getReview() != null) {
                            rbKkSelf.setClickable(false);
                            rbTimeEf.setClickable(false);
                            rbServiceSelf.setClickable(false);
                            etSelf.setEnabled(false);
                            //付款及时
                            rbEffectiveSelf.setStar(Float.parseFloat(review.getResult().getReview().getCostingGrade()));
                            rbTimeEf.setStar(Float.parseFloat(review.getResult().getReview().getTimingGrade()));
                            rbServiceSelf.setStar(Float.parseFloat(review.getResult().getReview().getServiceGrade()));
                            rbKkSelf.setStar(Float.parseFloat(review.getResult().getReview().getIntegrityGrade()));//出价慷慨
